...
kubectl create secret generic rebot-secret --namespace=rebot --dry-run=client --from-literal=DISCORD_TOKEN=$DISCORD_TOKEN --from-literal=DISCORD_SERVER_ID=$DISCORD_SERVER_ID --from-literal=ADMIN_API_KEY=$DISCORD_ADMIN_API_KEY -o yaml >k3s/rebot/rebot-secret-template.yml
